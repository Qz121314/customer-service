...
visitorPromotionApi.put('/api/admin/visitor-promotion', async (c) => {
  if (!(await adminAuthorized(c))) return unauthorized(c);
  const body = await readJson<PromotionInput>(c.req.raw);
  ...
  if (existing) {
    await c.env.DB.prepare(
      ...
      .bind(
        ...
        input.value.ctaLabel,
        input.value.ctaUrl,
        input.value.startsAt,
        input.value.endsAt,
        id,
      )
      .run();
...
visitorPromotionApi.get('/client/v1/promotion', async (c) => {
  ...
  if (!site) return clientError(c, 404, 'PROJECT_NOT_FOUND', 'Project was not found.');
...
visitorPromotionApi.put('/client/v1/promotion/:id/view', async (c) => {
  ...
  if (!id || !revision || !visitorId) {
    return clientError(c, 400, 'INVALID_PROMOTION_VIEW', 'Promotion view is invalid.');
  }
  ...
  let issuedVisitorToken: string | null = null;
  if (!visitor) {
    const created = await createPromotionVisitor(c.env.DB, site.id, visitorId);
    visitor = created.visitor;
    issuedVisitorToken = created.accessToken;
  }
...
async function loadPromotion(db: D1Database, siteId: string): Promise<PromotionRow | null> {
  return db
    .prepare(
...
function normalizePromotionInput(body: PromotionInput | null):
  ...
  const title = normalizeRequiredText(body.title, MAX_TITLE);
  const summary = normalizeRequiredText(body.summary, MAX_SUMMARY);
  const bodyMarkdown = normalizeRequiredText(body.bodyMarkdown, MAX_BODY);
  if (!title || !summary || !bodyMarkdown) {
    return { ok: false, error: 'INVALID_PROMOTION' };
  }

  const coverUrl = normalizeOptionalHttpUrl(body.coverUrl);
  if (coverUrl === undefined) return { ok: false, error: 'INVALID_PROMOTION_URL' };
  const ctaLabel = normalizeOptionalText(body.ctaLabel, MAX_CTA_LABEL);
  const ctaUrl = normalizeOptionalHttpUrl(body.ctaUrl);
  if (ctaLabel === undefined || ctaUrl === undefined || Boolean(ctaLabel) !== Boolean(ctaUrl)) {
    return { ok: false, error: 'INVALID_PROMOTION_CTA' };
  }
  const startsAt = normalizeOptionalTimestamp(body.startsAt);
  const endsAt = normalizeOptionalTimestamp(body.endsAt);
...
function normalizeOptionalHttpUrl(value: unknown): string | null | undefined {
  if (value === null || value === undefined || value === '') return null;
...
async function findSite(db: D1Database, projectId: string): Promise<{ id: string } | null> {
  return db
    .prepare(
      `SELECT id
       FROM sites
...
async function createPromotionVisitor(
  ...
  siteId: string,
  externalId: string,
): Promise<{ visitor: { id: string; site_id: string; external_id: string; expires_at: string | null; access_token_hash: string | null }; accessToken: string }> {
  const id = crypto.randomUUID();
  const accessToken = randomVisitorToken();
...
function clientError(c: Context<Env>, status: 400 | 401 | 404 | 409, code: string, message: string) {
