...
test(
  'bootstrap and refresh share one agent inbox loader without adding a request',
  async () => {
    ...
    assert.match(
      bootstrap,
      /inbox: await loadAgentInbox\(c\.env\.DB, agent\)/u,
    );
    assert.doesNotMatch(
      bootstrap,
      /loadBootstrapInbox|loadAgentQuotaOverview/u,
    );
