...
test('three equally eligible agents receive 300 normal conversations exactly evenly', async () => {
  ...
  for (let index = 1; index <= 300; index += 1) {
    await startConversation(instrumented.db, rooms, index, visitorId(index));
  }
...
test('CTA affinity does not consume or move the next normal round-robin turn', async () => {
  ...
  const first = await startConversation(instrumented.db, rooms, 1001, 'RET001');
  ...
  assert.deepEqual(
    new Map(
      receiptReasons.map((row) => [row.conversation_id, row.assignment_reason]),
    ),
    new Map([
...
test('busy agent is skipped and rejoins without catch-up priority', async () => {
  ...
  const first = await startConversation(instrumented.db, rooms, 2001, 'BSY001');
  assert.equal(first.agentName, 'agent-a');
  ...
  const third = await startConversation(instrumented.db, rooms, 2003, 'BSY003');
