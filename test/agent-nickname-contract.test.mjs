...
const avatar = readFileSync('src/dashboard/AgentAvatarControl.tsx', 'utf8');
...
test('agent nickname is self-service while the login username stays separate', () => {
  ...
  assert.match(panels, /onNicknameChange=\{onNicknameChange\}/u);
  assert.match(avatar, />客服资料</u);
  assert.match(avatar, />对外昵称</u);
  assert.match(avatar, /登录账号/u);
  assert.match(avatar, /onNicknameChange/u);
...
test('long admin agent directories scroll as one complete content surface', () => {
  ...
  assert.match(layout, /overscroll-behavior-y: auto/u);
});
