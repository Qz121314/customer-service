...
test('agent websocket presence avoids one D1 write per ping', async () => {
  ...
  assert.match(core, /datetime\(last_seen_at\) <= datetime\('now', '-90 seconds'\)/u);
  assert.match(routing, /datetime\(a\.last_seen_at\) >= datetime\('now', '-3 minutes'\)/u);
  assert.match(waiting, /datetime\(a\.last_seen_at\) >= datetime\('now', '-3 minutes'\)/u);
