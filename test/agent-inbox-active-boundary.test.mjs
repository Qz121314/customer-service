...
test(
  'default agent inbox never hides active conversations behind a global row limit',
  async () => {
...
test(
  'only closed conversations are bounded in explicit filtered inbox reads',
  async () => {
    const worker = await read('../src/worker/agent-api.ts');
    const start = worker.indexOf('async function loadAgentInbox');
    const end = worker.indexOf("agentApi.get('/api/agent/stats'", start);
...
test(
  'initial load, heartbeat and availability changes share one inbox loader',
  async () => {
    const worker = await read('../src/worker/agent-api.ts');
