...
```text
符合有效 CTA 两小时保护的客服
→ 当前产品上一位接待客服之后的第一个合格客服
→ 到客服 ID 顺序末尾后从最小 ID 环回
```

`routing_round_robin_cursors` 按 `site_id + product_id` 保存每个产品自己的 `last_agent_id`。不同产品的轮询互相隔离：客服在产品 A 接到流量，不会改变其在产品 B 的轮询位置。购买额度、每日接待上限、账号启用状态和在线状态只决定客服是否进入当前候选池，不提供任何权重或优先级。

忙碌、离线、停用、达到每日上限或额度耗尽的客服会被临时跳过；恢复资格后按原本的客服 ID 环形位置重新参与，不获得补偿式或追赶式优先级。候选选择和会话写入仍在一条 D1 原子更新中完成，数据库触发器在同一写入路径推进当前产品的游标。

没有合格客服时，会话保持等待。等待恢复只挑选当前确实存在合格接待者的最老会话，每轮最多处理 10 个；无法分配的旧队头不会阻塞后面已经可以分配的会话。恢复后的最终接收者仍由同一个正式分流函数决定，不存在第二套分流算法。
