...
- 最近会话已经关闭但仍在 2 小时内时，新建独立会话，但只允许原客服继续接待，不参与其他客服的候选排序；
- 原客服暂时离线、停用、超出容量或不再负责该产品时，新会话保持等待，不会在保护期内改派其他客服；
...
- 流量凭证与每日客服统计独立于 24 小时聊天数据，保留当前业务日及此前 399 天，共 400 个自然日；
...
- 管理中心“流量统计”按产品汇总首次有效接待，展示产品贡献占比、排行和整体每日趋势；
- 客服账号列表的“统计”按钮只查询该客服一个自然月的每日接待量，不加载产品维度；
- 产品月报和单客服月报各使用一次有日期边界的 D1 聚合查询，避免列表批量查询。
...
- 防滥用窗口记录定期清理；
- 每日客服统计 / 流量凭证每天按 `America/Los_Angeles` 业务日清理，固定保留 400 个自然日。
...
- `0039_cta_hard_agent_affinity.sql` 将关闭后新会话的原客服关联改为两小时硬约束，不再降级分配其他客服。
