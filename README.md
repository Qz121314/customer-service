...
> 这个项目不是长期 CRM。它的核心职责是把 Site / 产品页产生的有效咨询，按产品负责范围、坐席在线状态、并发容量、每日上限和已购买额度，稳定地分配给具体客服，并提供 24 小时临时服务周期。
...
服务端以 `siteId + visitorId + productId` 识别同一个产品咨询服务周期，`conversations.expires_at` 是唯一时间边界：

- 同一产品只要当前周期尚未过期，原会话仍为新会话或处理中时，CTA 直接返回原会话和原客服；即使中间数小时没有新消息，也不会因为“最后活动时间”重新分流；
- 当前会话已经关闭但周期尚未结束时，可以创建独立的新线程，但新线程继承原来的 `expires_at`，不会重新获得 24 小时；
- 关闭后新线程硬绑定上一线程最后实际负责的客服，自动分流、登录恢复和 heartbeat 批量认领等待会话都必须遵守该绑定；
- 绑定客服暂时离线、暂停接待、超出容量或当前不可接待时，会话保持等待，不自动降级给其他客服；
- 人工明确转接会把周期负责人移动到新客服；客服主动“重新进入自动分流”属于人工覆盖，会清除当前绑定并排除自己后重新选择；
- 不同产品彼此独立；只有 `expires_at` 到期后，下一次 CTA 才开始新的 24 小时周期并重新参与正常分流；
- 保护时间固定，不因发送新消息而滑动续期；
- 复用不会重复创建 D1 会话、重复消耗访客 / 来源创建额度、坐席流量额度或重复发送首次分配通知；
- 每个 CTA handoff 仍单独保存幂等映射，因此任何一次网络重试都能返回它最初对应的会话；
- D1 唯一开始键和原子额度凭证共同处理多标签页 / 并发点击；过期但尚未被 Cron 清理的旧开始键会在新周期插入前同步释放，不阻塞下一周期。

## 7. 会话生命周期与计数

### 7.1 24 小时临时服务周期

- 同一 `siteId + visitorId + productId` 的服务周期从该周期首个会话创建时固定计算 24 小时；
- `conversations.expires_at` 是运行时唯一权威到期字段，也是客服保护、CTA 复用和周期结束的统一边界；
- 周期内原线程关闭后创建的新线程继承原 `expires_at`，不会重新开始 24 小时；
- 新消息不续期，保护时间也不随 `last_message_at` 滑动；
- 热路径直接按 `expires_at` 查询，保留 `idx_conversations_expiry` 的索引价值；
- 到期后删除会话、消息、媒体记录和相关 R2 对象；
- 无会话的过期临时访客随后删除；
- Worker Cron 每 5 分钟进行一次有界清理；
- 即使 Cron 尚未清理过期记录，新周期也可以立即开始；
- 24 小时用于覆盖一次临时咨询服务周期，不代表长期客户历史。
...
- 同一访客重复点击同一产品 CTA 时，当前 24 小时服务周期的开始凭证保证并发请求只消费一次对应的幂等计数；
...
- `0038_cta_conversation_reuse.sql` 历史上引入 CTA 复用键、额度幂等凭证和多 handoff 映射，当时采用两小时窗口；
- `0039_cta_hard_agent_affinity.sql` 历史上增加关闭后新会话的原客服硬约束和独立到期字段；
- `0040_unified_service_cycle.sql` 删除独立 affinity 到期字段，将 CTA 复用、客服保护、等待恢复和周期结束统一到 `conversations.expires_at` 的 24 小时服务周期，并补齐人工转接 / 重新分流与过期开始键处理。
