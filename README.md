...
- 唯一例外是系统级恢复场景，例如原客服被删除导致已接待会话失去有效归属。恢复分配仍走正式路由函数，但不把同一笔咨询重新计费。
...
没有任何启用范围命中当前产品时，会话保持等待，不回退到旧客服分组模型，也不会随机分给无负责范围的客服。
...
**Busy 是不可被任何优先级覆盖的硬约束：只要客服当前 `status = busy`，就绝对不能接收任何新的自动分配。普通严格轮询、已关闭会话的 CTA 两小时亲和、等待会话恢复、额度恢复或其他系统恢复流程，都不能绕过 Busy。Busy 客服只能继续处理已经分配给自己的已有会话；只有客服本人重新切换为 `online` 后，才重新获得新流量候选资格。**
...
| 忙碌 `busy` | **绝对否** | 是 | Busy 是硬排除条件，任何亲和、恢复或轮询规则都不得绕过 |
...
C. 新会话候选筛选
   ...
   → status = busy 必须在这里被硬排除，不允许进入后续亲和或轮询候选

D. CTA 两小时客服亲和
   两小时内最近会话已经关闭，但曾有原客服
   → 只有原客服当前仍属于合格候选时，才优先选择原客服
   → 原客服当前 busy 时，亲和必须失效并进入普通轮询
   → 原客服其他资格不合格时，同样进入普通轮询
...
这里必须明确：**这不是 Busy 客服接到了“新会话”，而是原有活动会话继续保持原归属。**
...
- 原客服只要当前 `busy`，即使仍在两小时亲和窗口，也**绝对不能**接这条新会话；
- 原客服如果已经 offline、disabled、范围不匹配、达到日上限或额度耗尽，同样不能依靠两小时亲和绕过资格门槛；
...
- **Busy 客服仍然不具备恢复分配资格，系统恢复也不能绕过 `status = online` 的硬门槛。**
...
- 只处理当前确实已经出现合格接待者的等待会话；
- `busy` 客服不属于合格接待者，不能因为等待时间较长而作为兜底客服；
- 优先处理最老的可分配会话；
- 每轮最多处理 10 个；
- 某个老会话因为没有匹配客服而仍不可分配时，不能阻塞后面已经可以分配的会话；
- 每一个恢复会话最终仍调用同一个正式分流函数；
- 不允许为“等待恢复”维护第二套客服选择算法；
- 如果恢复时仍无合格客服，继续等待且不推进产品游标。
...
- **`busy` 客服通过任何路径收到新的自动分配，包括普通轮询、CTA 已关闭会话亲和、等待恢复和系统恢复；**
- `offline` 客服仍收到新的自动流量；
- 管理员停用的客服仍收到新的自动流量；
...
- 等待恢复只选择当前存在合格接待者的最老会话，每轮最多 10 个，不让不可分配的队头阻塞后续流量；
- R2 使用稳定对象键和有界批量清理；
...
禁用客服被跳过
忙碌客服在普通轮询、CTA 已关闭会话亲和、等待恢复和系统恢复中都绝对不能收到新分配
离线客服被跳过
负责范围不匹配被跳过
购买额度耗尽被跳过
