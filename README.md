...
- 已经产生不可变首次接待凭证的会话，如果因为原客服被删除而由系统恢复分配，不属于第二笔新流量，不重复消耗每日接待次数。
...
已经生成不可变首次接待凭证的会话，如果因为系统异常或原客服删除需要恢复归属：

- 不是第二笔新咨询；
- 不重复扣购买额度；
- 不重复增加每日接待计数；
- 因为它不是新流量，恢复分配不能被“已经为这笔咨询付过的额度”再次收费；
- 但接收恢复会话的客服仍必须满足账号启用、当前在线、负责范围命中和具备登录凭证等实际可接待条件。
