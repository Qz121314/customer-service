...
没有任何启用范围命中当前产品时，本次 CTA 直接返回后台配置的“无人可接提示”，不创建会话、不回退到旧客服分组模型，也不会随机分给无负责范围的客服。
...
G. 无合格客服
   → 返回 NO_AGENT_AVAILABLE 和后台配置提示
   → 不保留会话，不消耗访客 / 来源创建次数或客服流量额度
   → 不推进任何轮询游标
```
...
- 所有匹配客服都达到上限或其他资格不满足时，不创建新会话，直接返回后台配置的提示；
...
不创建 Conversation
→ 不创建消息或 handoff 归属
→ 不消耗 visitor / source 的 24 小时创建次数
→ 不消耗客服购买额度或每日接待次数
→ 返回 NO_AGENT_AVAILABLE 和 sites.no_agent_message
```

管理员可在客服后台修改 `noAgentMessage`，例如填写营业时间。Storefront 必须把服务端返回的文案作为纯文本弹窗展示。

客服后来上线、恢复额度、进入下一业务日或重新启用，只影响此后新发起的 CTA；系统不保存也不补偿此前失败的咨询尝试。活动会话复用规则保持不变，已经分配的会话不会因为原客服暂时失去新流量资格而被转走。
...
- 无人可接的 CTA 失败路径用一个 D1 批次清理临时会话与创建计数，不保留等待队列或恢复扫描；
- R2 使用稳定对象键和有界批量清理；
