...
- 按会话创建时间和稳定 ID 顺序，用 keyset 批次从老到新扫描等待会话；
- 每一条被扫描的等待会话都调用同一个正式分流函数，由它统一执行资格过滤、CTA 亲和和产品级严格轮询；
- 每轮最多成功恢复 10 个会话，未分配成功的队头不会消耗这个成功名额，也不会终止后续扫描；
- 因此，即使最旧 10 条当前都没有合格客服，后面的可分配会话仍会在同一轮扫描中被处理；
...
- 等待恢复按稳定 keyset 批次扫描，每条复用正式分流函数，每轮最多成功恢复 10 个且不让不可分配的队头阻塞后续流量；
- R2 使用稳定对象键和有界批量清理；
