...
产品目录以 Site 的 `/integration/v1/verify` 全量同步结果为权威。访客请求只提交 `product.id`；客服系统从 `product_catalog` 读取标题、链接、图片、分区和分类，不能通过访客请求覆盖目录或重新启用已禁用产品。Conversation Start、分流、额度和无客服处理都只围绕 Site 产品目录运行。
...
正式路由表为 `agent_routing_scopes`，使用现有 section/category/product 三种 Scope 类型和同一个 site-global strict round robin。访客通过现有 `POST /client/v1/conversations` 进入统一 Conversation Start / Routing / Quota / No-Agent 链路。
...
- 在首次问候语设置中维护最多 10 个 CTA 按钮；访客点击后，系统将按钮文案和对应自动回复持久化到当前会话；
- 消息已读状态和双方输入状态；
- 商品上下文；
...
CTA 属于坐席个人的首次问候语配置，不由管理员统一维护。每个 CTA 包含一个访客可见的按钮文案和一段自动回复；仅当当前会话实际发送了该坐席的首次问候语时，启用的 CTA 才会紧贴该问候消息显示给访客。访客点击后，按钮文案作为客户消息、自动回复作为坐席消息，在一个原子写入中共同保存并实时同步到双方；重试会复用同一对消息，不重复更新会话或发送提醒。
