...
- 在坐席设置中维护最多 10 组点击式快捷问答；访客点击问题后，系统将问题和对应自动答案持久化到当前会话；
- 消息已读状态和双方输入状态；
- 商品上下文；
...
快捷问答属于坐席个人配置，不由管理员统一维护。每组问答包含一个访客可见的问题和一段自动答案；只有当前会话已分配坐席的启用问答才会展示给访客。访客点击后，问题作为客户消息保存，答案作为 `system / auto_reply` 消息保存并实时同步到访客端和坐席端。点击请求使用客户端消息 ID 和答案消息 ID 幂等，网络重试不会重复生成答案。
