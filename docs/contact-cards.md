...
移动端名片面板只允许正文拥有一个纵向滚动容器：标题栏保持固定且“名片”标题严格水平居中；已保存名片按纵向列表排列，每一行向左滑动后才显示“编辑 / 删除”，操作按钮不能常驻挤压内容，也不能把整个列表做成横向卡片轮播。渠道下拉展开后参与正文布局，避免被弹窗边界裁切。保存成功后正文回到顶部完整展示新名片；点击编辑时表单滚动到可见区域。底部主操作在表单范围内保持可见，并兼容安全区与软键盘后的动态视口。
...
10. 390×844 移动端视口中弹窗不越界、标题居中、已保存名片列表本身不横向溢出、每一行具备独立左滑操作区，保存后滚动位置复位且名片行不与编辑器重叠。
