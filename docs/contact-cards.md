...
移动端名片面板只允许正文拥有一个纵向滚动容器：标题栏保持固定，已保存名片改为横向滑动卡片，不能再为名片列表增加第二个纵向滚动条。渠道下拉展开后参与正文布局，避免被弹窗边界裁切。保存成功后正文回到顶部完整展示新名片；点击编辑时表单滚动到可见区域。底部主操作在表单范围内保持可见，并兼容安全区与软键盘后的动态视口。
...
10. 390×844 移动端视口中弹窗不越界、已保存名片无纵向溢出、名片列表可横向滑动，保存后滚动位置复位且名片行不与编辑器重叠。
